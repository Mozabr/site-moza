...
(function () {
  ...
  /* ---- O traço: a mesma medida, desenrolada no tempo ---- */
  var faixa = document.querySelector('[data-traco]');
  var traco = faixa ? new TracoMoza(faixa) : null;
  ...
  if (cubo && etapas.length) {
    var marcarEtapa = function (alvo) {
      ...
      var d = parseFloat(alvo.dataset.desvio);
      cubo.paraDesvio(d);
      if (traco) traco.paraDesvio(d);
    };
